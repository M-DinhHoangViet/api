...
const livariants = 'https://livariants.org';
...
export default class Ctrl {
  ...
  constructor(readonly login: ServerLogin, readonly redraw: () => void) {
    ...
    fetch(`${livariants}/api/stream/event`, { headers: this.authorization })
      .then(res => {
  ...
  gameStart = (id: string) =>
    fetch(`${livariants}/api/board/game/stream/${id}`, { headers: this.authorization })
